...
TEST_F(TaskManagerTest, Constructor_Success) {
    ...
    std::vector<Task> test_tasks = {Task("Task 1", false), Task("Task 2", true)};

    CreateConfigFile(output_dir_.string(), "test_list.json");
    CreateTaskFile(test_tasks);

    // Тестирование конструктора
    EXPECT_NO_THROW({ TaskManager manager(config_file_.string()); });
}
...
TEST_F(TaskManagerTest, AddTask_String_Success) {
    ...
    EXPECT_NO_THROW({ manager.AddTask("New Task"); });
...
TEST_F(TaskManagerTest, AddTask_TaskObject_Success) {
    ...
    EXPECT_NO_THROW({ manager.AddTask(new_task); });
...
TEST_F(TaskManagerTest, ToggleTask_Success) {
    ...
    EXPECT_NO_THROW({ manager.ToggleTask(1); });
...
TEST_F(TaskManagerTest, ToggleTask_InvalidIndex_Throws) {
    ...
    EXPECT_THROW({ manager.ToggleTask(5); }, std::out_of_range);
}
...
TEST_F(TaskManagerTest, RemoveTask_Success) {
    ...
    EXPECT_NO_THROW({ manager.RemoveTask(1); });
...
TEST_F(TaskManagerTest, RemoveTask_InvalidIndex_Throws) {
    ...
    EXPECT_THROW({ manager.RemoveTask(5); }, std::out_of_range);
}
...
TEST_F(TaskManagerTest, EditTask_Success) {
    ...
    EXPECT_NO_THROW({ manager.EditTask(1, "Updated Task 2"); });
...
TEST_F(TaskManagerTest, EditTask_InvalidIndex_Throws) {
    ...
    EXPECT_THROW({ manager.EditTask(5, "New Task"); }, std::out_of_range);
}
...
TEST_F(TaskManagerTest, LoadTasksFromFile_Success) {
    ...
    TaskManager manager(config_file_.string());
    EXPECT_NO_THROW({ manager.LoadTasksFromFile(task_file_.string()); });
...
TEST_F(TaskManagerTest, LoadTasksFromFile_NonExistentFile) {
    ...
    TaskManager manager(config_file_.string());
    EXPECT_NO_THROW({ manager.LoadTasksFromFile(non_existent_file.string()); });
...
TEST_F(TaskManagerTest, SetPath_Success) {
    ...
    EXPECT_NO_THROW({ TaskManager::SetPath(new_path, config_file_.string()); });
...
TEST_F(TaskManagerTest, SetName_Success) {
    ...
    EXPECT_NO_THROW({ TaskManager::SetName(new_name, config_file_.string()); });
...
TEST_F(TaskManagerTest, MakeDefaultConfig_Success) {
    ...
    EXPECT_NO_THROW({ MakeDefaultConfig(); });
...
TEST_F(TaskManagerTest, EmptyTaskList) {
    ...
    EXPECT_THROW({ manager.ToggleTask(0); }, std::out_of_range);

    EXPECT_THROW({ manager.RemoveTask(0); }, std::out_of_range);

    EXPECT_THROW({ manager.EditTask(0, "New Task"); }, std::out_of_range);
}
...
TEST_F(TaskManagerTest, AddTask_EmptyString) {
    ...
    TaskManager manager(config_file_.string());
    EXPECT_THROW(manager.AddTask(""), std::invalid_argument);
...
TEST_F(TaskManagerTest, AddTask_VeryLongString) {
    ...
    EXPECT_THROW(manager.AddTask(long_text), std::invalid_argument);
...
TEST_F(TaskManagerTest, AddTask_SpecialCharacters) {
    ...
    EXPECT_NO_THROW({ manager.AddTask(special_text); });
...
TEST_F(TaskManagerTest, SaveAndLoad_ManyTasks) {
    ...
    EXPECT_NO_THROW({ manager2.LoadTasksFromFile(task_file_.string()); });
...
TEST_F(TaskManagerTest, ClearTasks_NonEmptyList_Success) {
    ...
    EXPECT_NO_THROW({ manager.ClearTasks(); });
